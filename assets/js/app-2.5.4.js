/*
     Official Website : https://netspeed.bytescrafter.net | Email: support@netspeed.bytescrafter.net
     Developed by : Vishnu | https://Vishnu.Pro | Email : me@vishnu.pro 
     Like this Project? Please Donate NOW & Keep us Alive -> https://go-netspeed.bytescrafter.net/Donate
    Speed Test by NetSpeed™️ is Free and Open-Source Software (FOSS) with MIT License.
    Read full license terms @ http://go-netspeed.bytescrafter.net/License
    If you have any Questions, ideas or Comments Please Send it via -> https://go-netspeed.bytescrafter.net/SendMessage
*/ 
window.onload = function() {
  var appSVG = document.getElementById("NetSpeed-UI");
...
(function(NetSpeed) {
  ...
  NetSpeedShow.prototype.reset = function() {
    this.element = "";
    this.chart = "";
    this.polygon = "";
    this.width = 200;
  ...
  NetSpeedShow.prototype.ip = function() {
    var Self = this;
  ...
  NetSpeedShow.prototype.prePing = function() {
    this.loader.fade("out", 500);
    this.NetSpeed.fade("in", 1000);
  };
  NetSpeedShow.prototype.app = function() {
    this.loader.fade("out", 500, this.ShowAppIntro());
  };
  NetSpeedShow.prototype.ShowAppIntro = function() {
    this.NetSpeed.fade("in", 1000);
  };
  NetSpeedShow.prototype.userInterface = function() {
    var Self = this;
    this.intro_Desk.fade("out", 1000);
    this.intro_Mob.fade("out", 1000, this.ShowUI());
  };
  NetSpeedShow.prototype.ShowUI = function() {
  ...
  NetSpeedShow.prototype.Symbol = function(dir) {
    if (dir == 0) {
  ...
  NetSpeedShow.prototype.Graph = function(speed, select) {
  ...
  NetSpeedShow.prototype.progress = function(Switch, duration) {
    var Self = this;
    var Stop = duration;
    var Stage = Switch;
  ...
  NetSpeedShow.prototype.mainGaugeProgress = function(currentSpeed) {
    var Self = this;
  ...
  NetSpeedShow.prototype.showStatus = function(e) {
    this.oDoLiveStatus.el.textContent = e;
  };
  NetSpeedShow.prototype.ConnectionError = function() {
    this.ConnectErrorMob.el.style.display = "block";
    this.ConnectErrorDesk.el.style.display = "block";
  };
  NetSpeedShow.prototype.uploadResult = function(upload) {
    if (upload < 1) {
  ...
  NetSpeedShow.prototype.pingResults = function(data, Display) {
    var ShowData = data;
    if (Display === "Ping") {
  ...
  NetSpeedShow.prototype.downloadResult = function(download) {
    if (download < 1) {
  ...
  NetSpeedShow.prototype.jitterResult = function(data, Display) {
    var ShowData = data;
    if (Display === "Jitter") {
      if (ShowData >= 1 && ShowData < 10000) {
  ...
  NetSpeedShow.prototype.LiveSpeed = function(data, Display) {
    var ShowData = data;
  ...
  NetSpeedShow.prototype.GaugeProgresstoZero = function(currentSpeed, status) {
    var speed = currentSpeed;
    var Self = this;
    var duration = 3;
    if (speed >= 0) {
  ...
  NetSpeedShow.prototype.getNonlinearDegree = function(mega_bps) {
    var i = 0;
  ...
  var NetSpeedGet = function() {
    this.OverAllTimeAvg = window.performance.now();
    this.SpeedSamples = [];
    this.FinalSpeed;
  };
  NetSpeedGet.prototype.reset = function() {
    this.OverAllTimeAvg = window.performance.now();
    this.SpeedSamples = [];
    this.FinalSpeed = 0;
  };
  NetSpeedGet.prototype.ArraySum = function(Arr) {
    var array = Arr;
    if (array) {
  ...
  NetSpeedGet.prototype.AvgSpeed = function(Livespeed, Start, duration) {
    var Self = this;
  ...
  NetSpeedGet.prototype.uRandom = function(size, callback) {
    var size = size;
  ...
  NetSpeedGet.prototype.addEvt = function(o, e, f) {
    o.addEventListener(e, f);
  };
  NetSpeedGet.prototype.remEvt = function(o, e, f) {
    o.removeEventListener(e, f);
  };
  var NetSpeedEngine = function() {
    var Get = new NetSpeedGet();
    var Show = new NetSpeedShow();
    Show.app();
    var SendData;
    ...
    if (selectServer) {
      if (typeof getCommand.host === "string" || typeof getCommand.h === "string") {
        ...
        if (isValidHttpUrl(severAddress)) {
          NetSpeedServerList = [{ServerName:"Home", Download:severAddress + "/downloading", Upload:severAddress + "/upload", ServerIcon:"DefaultIcon",},];
        }
      }
    }
    ...
    var NetSpeedRun = parseInt(getCommand.run);
    var NetSpeedRunR = parseInt(getCommand.r);
    var NetSpeedStart;
    if (enableRun) {
      if (typeof getCommand.run === "string" || typeof getCommand.r === "string") {
        if (NetSpeedRun > 0) {
          NetSpeedStart = NetSpeedRun;
        } else if (NetSpeedRunR > 0) {
          NetSpeedStart = NetSpeedRunR;
        } else {
          NetSpeedStart = 0;
        }
      }
    ...
    function runTasks() {
      ...
      if (NetSpeedStart >= 0) {
        ...
        Show.userInterface();
        init = false;
        var AutoTme = Math.ceil(Math.abs(NetSpeedStart));
      ...
      function countDownF() {
        if (AutoTme >= 1) {
          ...
          if (AutoTme <= 0) {
            ...
            launch = true;
            NetSpeedStart = undefined;
            runTasks();
          }
        }
      }
      if (NetSpeedServerList === "fetch" && launch === true) {
        launch = false;
    ...
    function NetSpeed() {
      ...
      var Engine = setInterval(function() {
        ...
        if (Status === "Error") {
          ...
          dummyElement.innerHTML = '<a xlink:href="https://netspeed.bytescrafter.net/FAQ.php?ref=NetworkError" style="cursor: pointer" target="_blank"></a>';
        ...
        if (Status === "SendR") {
          Show.showStatus("All done");
          var dummyElement = document.createElement("div");
          dummyElement.innerHTML = '<a xlink:href="https://netspeed.bytescrafter.net?ref=Self-Hosted-Outro&run=5" style="cursor: pointer" target="_blank"></a>';
          var htmlAnchorElement = dummyElement.querySelector("a");
    ...
    var pingServer = [];
    var finalJitter = [];
    var pingSendLength = NetSpeedServerList.length;
    function readServerList() {
      pingSendLength = NetSpeedServerList.length;
      Status = "Ping";
      performance.clearResourceTimings();
      if (pingSendStatus < pingSendLength - 1) {
        pingSendStatus++;
        if (statusPingTest != "Stop") {
          sendPingRequest(NetSpeedServerList[pingSendStatus], readServerList);
        }
      } else {
    ...
    var ServerConnect = function(auth) {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          if (auth == 6) {
            NetSpeedServerList = JSON.parse(return_data);
            launch = true;
            runTasks();
          }
          if (auth == 7) {
  ...
  NetSpeed.Start = function() {
    new NetSpeedEngine();
  };
})(window.NetSpeed = window.NetSpeed || {});
